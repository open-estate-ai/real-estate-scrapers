#!/usr/bin/env python3
"""
Deploy scraper service to AWS App Runner
...
def main():
    print("UP RERA Scraper App Runner Service - Docker Deployment")
    print("===========================================")
    ...
    ecr_repository = "up-rera-scraper-app-runner"
    ...
    try:
        ...
        ecr_url = run_command(
            ["./tf-infra-wrapper.sh", "dev", "output", "-raw", "ecr_repository_url"],
            capture_output=True
        )
        app_runner_service_name = run_command(
            ["./tf-infra-wrapper.sh", "dev", "output",
                "-raw", "app_runner_service_name"],
            capture_output=True
        )
    finally:
    ...
    if not ecr_url:
        print(
            "Error: ECR repository not found. Run './tf-infra-wrapper.sh dev apply' first.")
        sys.exit(1)

    print(f"ECR Repository: {ecr_url}")

    if not app_runner_service_name:
        print("Error: App Runner service name not found. Run './tf-infra-wrapper.sh dev apply' first.")
        sys.exit(1)
    print(f"App Runner Service Name: {app_runner_service_name}")
    ...
    login_cmd = ["docker", "login", "--username",
                 "AWS", "--password-stdin", ecr_url]
    ...
    print("\nNext step: Run './tf-infra-wrapper.sh dev apply' in terraform to create the App Runner service.")
    ...
    try:
        services = run_command([
            ...
            "--region", region,
            "--query", f"ServiceSummaryList[?ServiceName=='{app_runner_service_name}'].ServiceArn",
            "--output", "json"
        ], capture_output=True)
